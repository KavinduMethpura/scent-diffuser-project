...
export async function GET() {
  ...
  if (!accessToken) {
    return Response.json(
      { ok: false, error: 'Not authenticated. Visit /api/auth/login first.' },
      { status: 401 }
    );
  }
  ...
  if (provider === 'google') {
    // -------------------------------------------------------------
    // Google Health API (Heart Rate DataPoints)
    // -------------------------------------------------------------
    ...
    if (res.status === 401) {
      try {
        ...
        return Response.json({ ok: false, error: refreshErr.message }, { status: 401 });
      }
    }

    if (!res.ok) {
      const errText = await res.text();
      return Response.json({ ok: false, error: `Google Health API (${res.status}): ${errText}` }, { status: 502 });
    }
    ...
    if (!res.ok) {
      const errText = await res.text();
      return Response.json({ ok: false, error: `Fitbit API (${res.status}): ${errText}` }, { status: 502 });
    }
  ...
  if (bpmSeries.length === 0 || currentBpm === null) {
    ...
    return Response.json({ ok: true, status: fallbackStatus });
  }
  ...
  return Response.json({ ok: true, ...statusPayload });
}
