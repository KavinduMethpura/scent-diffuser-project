...
// Fallback local file storage (uses writable /tmp on Vercel serverless if KV is not yet attached)
const storageDir = process.env.VERCEL ? '/tmp' : process.cwd();
const localFile = path.join(storageDir, '.local_kv_store.json');
